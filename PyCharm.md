➡️ cd ~/Jetbrains
sudo cp -R ~/Jetbrains/pycharm-2024.1.7 /opt/pycharm
sudo ln -s /opt/pycharm/bin/pycharm.sh /usr/local/bin/pycharm

➡️ # Logo @See: https://www.jetbrains.com/company/brand/
wget -O - https://resources.jetbrains.com/storage/products/company/brand/logos/PyCharm_icon.zip | tar -zxp

sudo cp PyCharm_icon.png /opt/pycharm/

➡️ sudo micro /usr/share/applications/pycharm.desktop

---

[Desktop Entry]
Version=1.0
Type=Application
Name=PyCharm
Icon=/opt/pycharm/PyCharm_icon.png
Exec="/opt/pycharm/bin/pycharm.sh"
Comment=IDE for Python
Categories=Development;IDE;
Terminal=false
StartupWMClass=pycharm

---

➡️ Активация 
1️⃣ Здесь https://3.jetbra.in/ найти подходящий сервер, перейти по ссылке и скачать архив, распаковать его.

2️⃣ Запустить файл
sh jetbra/scripts/install.sh
Сообщение "done. you'd better log off first!"

3️⃣ Ищем PyCharm
- Копируем код
- Запускаем PyCharm
- Вставляем код в поле ввода кода при активации
- Применяем

4️⃣ reboot

---
GAJWL09BT5RSXDR---